...
public class FreeBoardController {
	...
	//파라미터가 여러개일 때는 VO
	@RequestMapping("/more/freeList.wow")
	public String freeList(Model model, @ModelAttribute("searchVO") FreeBoardSearchVO searchVO) {
		...
		return "more/freeList";
	}
	
	
	
	
	@RequestMapping("/more/freeView.wow")
	//파라미터가 한개일 때는 변수쓰기
	public String freeView(Model model, @RequestParam(required = false) int boNo) {
		
		try{
			FreeBoardVO freeBoard=freeBoardService.getBoard(boNo);//getBoard할 때 attach도 같이 
			freeBoardService.increaseHit(boNo);
			model.addAttribute("freeBoard", freeBoard);
			return "more/freeView";
		}catch (BizNotFoundException  | BizNotEffectedException  e){
			ResultMessageVO resultMessageVO=new ResultMessageVO();
			resultMessageVO.messageSetting(false, 
					"NotFound or NotEffected", "글이없다 or조회수증가실패", "/more/freeList.wow", "목록으로");
	...
	@RequestMapping("/more/freeEdit.wow")
	public String freeEdit(Model model, int boNo) {
		try{ 
			FreeBoardVO freeBoard=freeBoardService.getBoard(boNo);
			model.addAttribute("freeBoard", freeBoard);
			
			return "more/freeEdit";
		}catch(BizNotFoundException e){
			ResultMessageVO resultMessageVO=new ResultMessageVO();
			resultMessageVO.messageSetting(false, 
					"NotFound", "글이없다", "/more/freeList.wow", "목록으로");
			model.addAttribute("resultMessageVO", resultMessageVO);
			return "common/message";
		}
	...
	@PostMapping("/more/freeModify.wow")
	public String freeModify(Model model, @ModelAttribute("freeBoard")@Validated(value = {Modify.class}) FreeBoardVO freeBoard, BindingResult error,
		...
		try{
			...
			resultMessageVO.messageSetting(true, 
					"MODIFY", "수정성공", "/more/freeList.wow", "목록으로");
		}catch (BizNotFoundException bnf){
			resultMessageVO.messageSetting(false, 
					"NotFound", "수정실패", "/more/freeList.wow", "목록으로");
		}catch (BizNotEffectedException bne){
			resultMessageVO.messageSetting(false, 
					"NotEffected", "수정실패", "/more/freeList.wow", "목록으로");
		}catch(BizPasswordNotMatchedException bpn){
			resultMessageVO.messageSetting(false, 
					"비밀번호틀림", "수정실패", "/more/freeList.wow", "목록으로");
		}
		model.addAttribute("resultMessageVO", resultMessageVO);
		return "common/message";
	}	
...
//	}
	...
	@PostMapping("/more/freeDelete.wow")
	public String freeDelete(Model model, @ModelAttribute("freeBoad") FreeBoardVO freeBoard) {
		...
		try{
			...
			resultMessageVO.messageSetting(true, 
					"DELETE", "삭제성공", "/more/freeList.wow", "목록으로");
		}catch (BizNotFoundException bnf){
			resultMessageVO.messageSetting(false, 
					"NotFound", "삭제실패", "/more/freeList.wow", "목록으로");
		}catch (BizNotEffectedException bne){
			resultMessageVO.messageSetting(false, 
					"NotEffected", "삭제실패", "/more/freeList.wow", "목록으로");
		}catch(BizPasswordNotMatchedException bpn){
			resultMessageVO.messageSetting(false, 
					"비밀번호틀림", "비밀번호틀렸다 너.  글쓴사람 아니지?", "/more/freeList.wow", "목록으로");
		}
		model.addAttribute("resultMessageVO", resultMessageVO);
	...
	@RequestMapping("/more/freeForm.wow")
	public String freeForm(Model model, @ModelAttribute("freeBoard") FreeBoardVO freeBoard) {
		
		return "more/freeForm";
	}
	
	
	
	
	@PostMapping("/more/freeRegist.wow")
	public String freeRegist(Model model, @ModelAttribute("freeBoard")@Validated() FreeBoardVO freeBoard, BindingResult error
		...
		try {
			...
			resultMessageVO.messageSetting(true, 
					"REGIST", "등록성공", "/more/freeList.wow", "목록으로");
		} catch (BizNotEffectedException e) {
			resultMessageVO.messageSetting(false, 
					"NotEffected", "등록실패", "/more/freeList.wow", "목록으로");
		}
		model.addAttribute("resultMessageVO", resultMessageVO);
		return "common/message";
	}
	...
	@RequestMapping("/more/javas.wow")
	public String javascript(Model model) {
		return "more/javas";
	}
	
	@RequestMapping("/more/jqry.wow")
