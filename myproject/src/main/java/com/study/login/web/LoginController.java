...
public class LoginController {
	...
	// String referer = ""; 빈에서는 절대 데이터 저장 필드 사용 안함
	...
	public String loginPost(HttpServletRequest req, HttpServletResponse resp, HttpSession session) throws Exception {
		...
		if ((id == null || id.isEmpty()) || (pw == null || pw.isEmpty())) {
			return "redirect:" + "/login/login.wow?msg=" + URLEncoder.encode("아이디패스워드를입력해주세요", "utf-8");
			// 직접 redirect보다는 DS한테 나 redirect하고싶어 를 알려주는게...
		} else {
			// DB에 있는 member테이블을 조회해서 로그인가능여부판단.
			UserVO user = loginService.getUser(id);        //UserVO가 생김
			if (user == null) {
